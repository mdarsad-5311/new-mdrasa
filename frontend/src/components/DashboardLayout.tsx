...
export default function DashboardLayout({
  ...
  return (
    <div className={`min-h-screen bg-background flex overflow-hidden ${isUrdu ? "font-urdu" : "font-sans"}`}>
      ...
      {/* LEFT SIDEBAR */}
      <aside
        className={`fixed inset-y-0 ${isUrdu ? "right-0" : "left-0"} z-50 w-80 bg-primary text-white transform transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : isUrdu ? "translate-x-full" : "-translate-x-full"
          }`}
      >
        <div className="h-full flex flex-col pt-10 pb-8 px-6">
            ...
            {sidebarItems.map((item) => {
              ...
              return (
                <Link
                  key={item.name}
                  href={item.href}
                  className={`flex items-center gap-5 px-6 py-4 rounded-full transition-all group relative ${isActive ? "bg-white/10 shadow-lg" : "hover:bg-white/5"
                    }`}
                >
                  <item.icon className={`w-5 h-5 transition-colors ${isActive ? "text-accent" : "text-sage group-hover:text-white"
                    }`} />
                  <span className={`text-sm font-bold tracking-wide transition-colors ${isUrdu ? "font-urdu text-base" : ""} ${isActive ? "text-white" : "text-sage group-hover:text-white"
                    }`}>{item.name}</span>
                  {isActive && (
          ...
          <div className="pt-6 mt-6 border-t border-white/10 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button 
                onClick={logout} 
                title={t("side.logout") || "Logout"} 
                className={`p-2 text-sage hover:text-white hover:bg-white/10 rounded-xl transition-all group ${isUrdu ? "rotate-180" : ""}`}
              >
      ...
      <main className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        ...
        <header className="h-28 bg-[#FDFCF9]/80 backdrop-blur-md px-10 flex items-center justify-between relative z-30">
              ...
              {childSwitcherOpen && (
                <div className="absolute top-full left-0 mt-2 w-64 bg-white rounded-3xl shadow-premium border border-border p-3 space-y-1 animate-in fade-in slide-in-from-top-2">
                  {children_list.map((child, idx) => (
                    <button
                      key={idx}
                      className={`w-full flex items-center justify-between p-4 rounded-2xl transition-all ${child.active ? "bg-primary/5 border border-primary/10" : "hover:bg-background border border-transparent"
                        }`}
          ...
          <div className="flex items-center gap-6">
            {/* Search Bar */}
            <div className="hidden xl:flex items-center bg-white border border-border/40 px-8 h-14 rounded-full w-100 shadow-soft focus-within:shadow-premium transition-all">
            ...
            <div className="flex items-center gap-4">
              ...
              <Link 
                href={role === 'admin' ? "/admin/settings" : role === 'student' ? "/student/settings" : "/parent/settings"} 
