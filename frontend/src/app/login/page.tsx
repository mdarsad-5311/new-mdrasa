...
export default function LoginPage() {
  ...
  const handleLogin = async (e?: React.FormEvent, customCredentials?: { email: string; password: string; role: "student" | "parent" | "admin" }) => {
    if (e) e.preventDefault();
    setLoading(true);
    setError("");
    const credentials = customCredentials || { email, password, role };
    try {
  ...
  const handleDemoLogin = (demoRole: "student" | "parent" | "admin") => {
    setRole(demoRole);
    const demoEmail = `${demoRole}@mdrasa.edu`;
    const demoPassword = `${demoRole}123`;
    setEmail(demoEmail);
    setPassword(demoPassword);
    handleLogin(undefined, { email: demoEmail, password: demoPassword, role: demoRole });
  };
        ...
        <div className="w-full max-w-md space-y-12">
            ...
            {[
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setEmail(`${tab.id}@mdrasa.edu`);
                  setPassword(`${tab.id}123`);
                }}
                className={`flex flex-col items-center justify-center py-3 rounded-xl transition-all gap-1.5 ${role === tab.id
          ...
          <div className="bg-primary/5 p-4 rounded-2xl border border-primary/10 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-black uppercase tracking-widest text-accent">Quick Demo Logins</span>
              <span className="text-[10px] text-sage font-medium">1-Click Access</span>
            </div>
            <div className="grid grid-cols-3 gap-2">
              <button
                type="button"
                onClick={() => handleDemoLogin("admin")}
                className="px-3 py-2 bg-white hover:bg-primary hover:text-white text-primary text-[10px] font-bold uppercase tracking-wider rounded-xl border border-border/40 transition-all shadow-sm active:scale-95"
              >
                Admin ⚡
              </button>
              <button
