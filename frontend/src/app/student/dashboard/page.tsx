...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        setStats({
          attendance: user?.attendance || "94.5%",
          pendingFees: user?.pendingFees || "$45.00",
          course: user?.course || "Hifz Quran",
          nextExam: "May 20"
        });
      } catch (err: any) {
        console.error("Student Dashboard Auth Error:", err.message);
  ...
  return (
    <DashboardLayout 
      ...
      userProfile={{ 
        name: user?.name || (isUrdu ? "طالب علم" : "Student"), 
        roleName: isUrdu ? "طالب علم" : "Student", 
        avatar: "" 
      }}
    >
        ...
        <>
          ...
          <div className={`flex flex-col md:flex-row justify-between items-start md:items-end gap-8 bg-white p-8 md:p-12 rounded-4xl shadow-soft border border-beige/10 overflow-hidden relative group ${isUrdu ? "md:flex-row-reverse" : ""}`}>
             ...
             <div className="space-y-4 relative z-10 w-full md:w-auto">
                ...
                <h2 className={`text-4xl md:text-6xl font-serif font-bold text-primary leading-tight ${isUrdu ? "font-urdu text-5xl md:text-7xl" : ""}`}>
                  {isUrdu ? `السلام علیکم، ${user?.name?.split(' ')[0]}` : `Salaam, ${user?.name?.split(' ')[0]}`} 👋
                </h2>
                <p className={`text-primary/60 font-sans font-semibold text-sm tracking-widest uppercase flex items-center gap-3 ${isUrdu ? "font-urdu tracking-normal text-lg flex-row-reverse" : ""}`}>
                  {isUrdu ? "آئی ڈی" : "ID"}: #{user?._id?.slice(-8).toUpperCase()} <span className="text-accent/30 font-light">|</span> {user?.className || (isUrdu ? "حفظ القرآن" : "Hifz-ul-Quran")}
                </p>
             </div>
          ...
          <div className={`grid grid-cols-1 xl:grid-cols-12 gap-12 pt-8 ${isUrdu ? "rtl" : ""}`}>
             ...
             <div className="xl:col-span-5 space-y-8">
                ...
                <div className="grid grid-cols-1 gap-6">
                   {notices.length === 0 ? (
                      <p className="text-center py-10 opacity-30 italic">No new notices for you.</p>
                   ) : notices.map((alert, a) => (
                      <div key={a} className={`p-8 bg-white border border-beige/10 rounded-4xl shadow-sm hover:shadow-soft transition-all group flex items-start gap-6 cursor-pointer relative overflow-hidden ${isUrdu ? "flex-row-reverse text-right" : ""}`}>
                         ...
                         <div className="flex-1 space-y-3">
                            <div className={`flex items-center justify-between ${isUrdu ? "flex-row-reverse" : ""}`}>
                               <span className={`text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-full ${alert.status === 'Urgent' ? 'bg-red-100 text-red-600' : 'bg-primary/10 text-primary'} ${isUrdu ? "font-urdu tracking-normal text-[10px]" : ""}`}>{alert.category}</span>
                               <span className={`text-[10px] font-bold text-primary/30 flex items-center gap-1.5 ${isUrdu ? "flex-row-reverse" : ""}`}><Clock className="w-3 h-3" /> {new Date(alert.createdAt).toLocaleDateString()}</span>
                            </div>
                            ...
                            <p className={`text-xs text-primary/50 font-medium leading-relaxed line-clamp-2 italic ${isUrdu ? "text-sm" : ""}`}>"{alert.description}"</p>
                         </div>
                      </div>
                   ))}
                </div>
