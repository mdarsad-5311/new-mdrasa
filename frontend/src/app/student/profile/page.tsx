...
export default function ProfilePage() {
  ...
  const studentInfo = {
    ...
    rollNumber: user?.rollNumber || "#MN-2026-XXX",
    course: user?.course || "Hifz-ul-Quran (Level 2)",
    status: user?.status || "Active Scholar",
    ...
    admissionDate: user?.admissionDate || "N/A",
    enrolledCourse: user?.course || "Hifz-ul-Quran",
    currentLevel: user?.level || "Level 2",
    batchTiming: user?.batchTiming || "Morning",
    juzProgress: user?.progress || "0/30",
    attendance: user?.attendanceRate || "0%",
    lastExam: user?.lastExamScore || "N/A"
  };
