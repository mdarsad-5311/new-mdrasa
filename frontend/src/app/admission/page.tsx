...
export default function AdmissionPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = await api.submitAdmission({
        ...formData,
      ...
      <div className={`min-h-screen bg-transparent flex flex-col items-center justify-center p-8 text-center space-y-12 ${isUrdu ? "font-urdu" : "font-sans"}`}>
         ...
         <div className="bg-white p-8 rounded-4xl border border-black/5 shadow-premium space-y-4 max-w-md w-full">
            ...
            <div className="pt-4 flex flex-col gap-3">
               <Link href="/" className={`w-full h-14 bg-primary text-white font-black text-sm uppercase tracking-widest rounded-2xl shadow-xl hover:-translate-y-1 transition-all flex items-center justify-center ${isUrdu ? "text-base" : ""}`}>
                 {t("adm_p.back_home")}
               </Link>
               <button onClick={() => setSuccess(false)} className={`w-full h-14 bg-cream text-primary font-black text-sm uppercase tracking-widest rounded-2xl hover:bg-white transition-all cursor-pointer ${isUrdu ? "text-base" : ""}`}>
