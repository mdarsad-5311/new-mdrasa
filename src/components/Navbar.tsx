...
export default function Navbar() {
  ...
  return (
    ...
    >
            ...
            <div className={`flex gap-1 items-center bg-primary/5 p-1.5 rounded-2xl border border-primary/5 ${isUrdu ? "flex-row-reverse" : ""}`}>
              {navLinks.map((link) => (
                <Link
                  key={link.name}
                  href={link.href}
                  className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs font-bold text-primary hover:bg-primary-dark hover:text-white transition-all transform hover:-translate-y-0.5 whitespace-nowrap group ${isUrdu ? "font-urdu text-base flex-row-reverse" : ""}`}
                >
                  <span className="text-accent group-hover:text-accent transition-colors">{link.icon}</span>
                  {link.name}
                </Link>
              ))}
            </div>
      ...
      {/* Mobile Menu */}
      <div
        className={`xl:hidden absolute w-full bg-white shadow-2xl transition-all duration-500 ease-in-out border-b border-primary/5 overflow-hidden ${
        ...
        <div className="px-6 space-y-4">
          {navLinks.map((link) => (
            <Link
              ...
              className={`flex items-center gap-4 text-xl font-bold text-primary hover:bg-primary/5 py-4 border-b border-primary/5 px-4 rounded-xl transition-all ${isUrdu ? "font-urdu flex-row-reverse text-right" : ""}`}
            >
